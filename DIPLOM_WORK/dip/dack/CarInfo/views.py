...
from .models import Car, TO, Reclamation, CarModelReference, DvigModel, \
    TransmissionModelReference, MainBridgeModelReference, ControlledBridgeModelReference, TypeOfMaintenance, \
    RecoveryMethod, FailureNode
...
def get_machine_data(request):
    ...
    try:
        machine = Car.objects.get(factory_number=factory_number)
    except:
        return Response(status=status.HTTP_200_OK)

    result = {
        'users_data': Users.objects.filter(role="client").values('first_name'),
        'services_data': Users.objects.filter(role="service_organisation").values('first_name'),
        'filter_data': {
            'machine_models': CarModelReference.objects.all().values('name'),
            'engine_models': DvigModel.objects.all().values('name'),
            'transmission_models': TransmissionModelReference.objects.all().values('name'),
            'driving_bridge_models': MainBridgeModelReference.objects.all().values('name'),
...
def post_machine_data(request):
    ...
    try:
        ...
        machine_model = CarModelReference.objects.get_or_create(name=request.data['machine_model'])[0]
        engine_model = DvigModel.objects.get_or_create(name=request.data['engine_model'])[0]
        ...
        driving_bridge_model = \
            MainBridgeModelReference.objects.get_or_create(name=request.data['driving_bridge_model'])[0]
        ...
        client = Users.objects.get(first_name=request.data['client'])
        service_company = Users.objects.get(first_name=request.data['service_company'])

        machine = Car.objects.update_or_create(
            factory_number=factory_number,
            defaults={
...
def post_maintenance_data(request):
    ...
    try:
        type_of_maintenance = \
            TypeOfMaintenance.objects.get_or_create(name=request.data['type_of_maintenance'])[0]
        ...
        machine = Car.objects.get_or_create(factory_number=request.data['machine'])[0]

        maintenance = TO.objects.update_or_create(
...
def post_complaints_data(request):
    ...
    try:
        ...
        failure_node = FailureNode.objects.get_or_create(name=request.data['failure_node'])[0]
        id = request.data['id']
        recovery_method = RecoveryMethod.objects.get_or_create(name=request.data['recovery_method'])[0]
        ...
        if not isinstance(id, int):
            ...
            complaint = Reclamation.objects.create(
            ...
            complaint = Reclamation.objects.update_or_create(
                id=id,
                defaults={
                    'failure_description': failure_description,
                    'date_of_refusal': date_of_refusal,
...
def get_machine_list(request):
    ...
    machine_list = Car.objects.all()
    if request.user.role == 'client':
        machine_list = Car.objects.filter(client__username=request.user)
    if request.user.role == 'service_organisation':
        machine_list = Car.objects.filter(service_company__username=request.user)
    if request.user.role == 'manager':
        machine_list = Car.objects.all()
...
def get_maintenance_unit(request):
    ...
    maintenance = TO.objects.filter(id=maintenance_id)
    result = []
...
def get_maintenance(request):
    ...
    maintenances = TO.objects.none()
    machines = Car.objects.none()
    if request.user.role == 'client':
        maintenances = TO.objects.filter(machine__client__username=request.user)
        machines = Car.objects.filter(client__username=request.user).values('factory_number')
    if request.user.role == 'service_organisation':
        machines = Car.objects.filter(service_company__username=request.user).values('factory_number')
        maintenances = TO.objects.filter(machine__service_company__username=request.user)
    if request.user.role == 'manager':
        maintenances = TO.objects.all()
        machines = Car.objects.all().values('factory_number')
    ...
    for maintenance in maintenances:
        if maintenance.machine.service_company == request.user or maintenance.machine.client == request.user or request.user.role == 'manager':
            result = {
                ...
                'select_data': {
                    'machine': Car.objects.all().values('factory_number'),
                    'type_maintenance': TypeOfMaintenance.objects.all().values('name')
...
def get_complaints_unit(request):
    ...
    complaint = Reclamation.objects.filter(id=complaint_id)
    result = []
...
def get_complaints(request):
    ...
    complaints = TO.objects.all()

    if request.user.role == 'client':
        machines = Car.objects.filter(client__username=request.user).values('factory_number')
        complaints = Reclamation.objects.filter(machine__client__username=request.user)
    if request.user.role == 'service_organisation':
        machines = Car.objects.filter(service_company__username=request.user).values('factory_number')
        complaints = Reclamation.objects.filter(machine__service_company__username=request.user)
    if request.user.role == 'manager':
        machines = Car.objects.all().values('factory_number')

        complaints = Reclamation.objects.all()
    ...
    for complaint in complaints:
        if complaint.machine.service_company == request.user or complaint.machine.client == request.user or request.user.role == 'manager':
            ...
            result = {
                ...
                'select_data': {
                    'machine': Car.objects.all().values('factory_number').distinct(),
                    'failure_node': FailureNode.objects.all().values('name'),
                    'recovery_method': RecoveryMethod.objects.all().values('name')
                },
                'filter_data': {
